...
- **Модуль ядра WireGuard** (для резервного подключения при 504 ошибках)
  - В современных ядрах Linux (5.6+) встроен по умолчанию
  - Проверка: `lsmod | grep wireguard` или `sudo modprobe wireguard`
...
Скрипт автоматически создаст `.zip` архив с Docker-образами и файлами проекта.

### Структура созданного архива

В архиве — все образы и файлы проекта; после распаковки достаточно загрузить образы и запустить `docker compose up`.

**Linux/Mac:** архив `bottgdomains-offline-YYYYMMDD-HHMMSS.tar.gz`  
**Windows:** архив `bottgdomains-offline-YYYYMMDD-HHMMSS.zip`

```
├── images/
│   ├── gostsslcheck.tar   # образ GOST (один образ для gostsslcheck1/2/3)
│   ├── tgscanner.tar      # образ бота
│   └── wireguard.tar      # образ WireGuard (собран из ./wireguard)
└── project/
    ├── docker-compose.yml
    ...
    ├── bot/
    ├── gost/
    ├── wireguard/         # контекст сборки WireGuard (для справки)
    └── wg/                 # конфиг WireGuard (wg0.conf)
```
...
3. **Распакуйте архив** (появится каталог `images/` и `project/`):

```bash
tar -xzf bottgdomains-offline-*.tar.gz
```

4. **Перейдите в директорию проекта и запустите deploy**:
...
Скрипт `manage.sh deploy` автоматически:
...
1. **Распакуйте архив** (появится `images/` и `project/`):
...
2. **Загрузите Docker образы** (или используйте `./manage.sh deploy` — он загрузит все и создаст override):
...
5. **Запустите сервисы** (при ручной загрузке образов создайте `docker-compose.override.yml` с `image:` для всех сервисов, иначе compose попытается собрать образы заново). Проще использовать `./manage.sh deploy` — он загрузит образы, создаст override и запустит сервисы:
...
**Решение:**
1. Проверьте токен в `.env`: `grep TG_TOKEN .env`
...
### WireGuard (резерв при 504 ошибках)

WireGuard используется как резервное подключение, когда все GOST endpoints возвращают 504 ошибки.

**ВАЖНО:** Проект работает ТОЛЬКО в Docker контейнерах. WireGuard работает в **отдельном контейнере** (собственный образ из `wireguard/`), а не внутри контейнера `tgscanner`.

**Архитектура:**
- WireGuard работает в отдельном контейнере с собственными правами доступа
- Контейнер `tgscanner` подключен к сети WireGuard контейнера через Docker networks
- Собственный образ поднимает туннель без настройки sysctl на хосте (работает на любой системе)

**Требования к хосту для WireGuard:**
- Модуль ядра WireGuard должен быть в ядре Linux хоста (где запускается Docker)
- Проверка: `lsmod | grep wireguard` или `sudo modprobe wireguard`
- В современных ядрах Linux (5.6+) WireGuard встроен в ядро по умолчанию
- WireGuard контейнер использует модуль ядра хоста через `/dev/net/tun` (смонтирован в docker-compose.yml)

**Офлайн: образ WireGuard.** Команда `./manage.sh export` (или `.\manage.ps1 export` на Windows) собирает все образы, в том числе `bottgdomains-wireguard:latest` из `./wireguard`, и сохраняет их в архив (включая `images/wireguard.tar`). На целевом хосте `./manage.sh deploy` загружает образы, помечает wireguard как `bottgdomains-wireguard:latest` и создаёт `docker-compose.override.yml`, так что запуск идёт без сборки и без интернета.

**Настройка:**
1. Создайте файл `wg/wg0.conf` с конфигурацией WireGuard на хосте (образ ожидает имя **wg0.conf**; при необходимости скопируйте или переименуйте свой конфиг в `wg0.conf`)
2. WireGuard контейнер автоматически поднимет интерфейс при старте (если конфиг найден)
3. Админ-панель → «Проверить WireGuard» — проверка статуса контейнера
4. Контейнер управляется через docker-compose (не требует ручного управления интерфейсом)
...
- wireguard.tar: ~20-50 MB (Alpine + wireguard-tools)
