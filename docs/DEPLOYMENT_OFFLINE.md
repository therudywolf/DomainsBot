...
**Офлайн: образ WireGuard.** Соберите образ на машине с интернетом: `docker compose build wireguard`, затем сохраните: `docker save bottgdomains-wireguard:latest -o images/wireguard.tar`. На целевом хосте скрипт deploy.sh загрузит образ из `images/wireguard.tar` и создаст тег `bottgdomains-wireguard:latest` для override.
