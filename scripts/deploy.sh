...
# Создаем директорию wg/ для WireGuard конфига (если не существует)
if [ ! -d "wg" ]; then
    mkdir -p wg
    echo "✅ Директория wg/ создана (для WireGuard конфига)"
    echo "   ⚠️  ВАЖНО: Создайте файл wg/TGBOT.conf с конфигурацией WireGuard"
    echo "   ⚠️  Без конфига WireGuard резервное подключение при 504 ошибках не будет работать"
else
    echo "✅ Директория wg/ уже существует"
    if [ ! -f "wg/TGBOT.conf" ]; then
        echo "   ⚠️  ВАЖНО: Файл wg/TGBOT.conf не найден"
        echo "   ⚠️  Создайте конфиг WireGuard для работы резервного подключения"
    else
        echo "   ✅ Конфиг WireGuard найден: wg/TGBOT.conf"
    fi
fi
echo ""
