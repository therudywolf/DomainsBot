...
# Конфиг теперь встроен в образ, но можно переопределить через volume если нужно
if [ ! -d "wg" ]; then
    mkdir -p wg
    echo "✅ Директория wg/ создана (опционально, для переопределения конфига)"
    echo "   ℹ️  Конфиг WireGuard уже встроен в образ Docker"
    echo "   ℹ️  Если нужно переопределить конфиг, создайте wg/TGBOT.conf и раскомментируйте volume в docker-compose.yml"
else
    echo "✅ Директория wg/ уже существует"
    if [ -f "wg/TGBOT.conf" ]; then
        echo "   ✅ Конфиг WireGuard найден: wg/TGBOT.conf (будет использован вместо встроенного)"
    else
        echo "   ℹ️  Конфиг WireGuard встроен в образ, переопределение не требуется"
    fi
fi
echo ""
