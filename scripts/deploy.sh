...
if lsmod | grep -q wireguard 2>/dev/null || modprobe wireguard 2>/dev/null; then
    ...
    echo "   Или загрузите модуль: sudo modprobe wireguard"
fi
echo ""
...
services:
  ...
  # WireGuard контейнер использует готовый образ masipcat/wireguard-go:latest
  # Если нужен offline режим, образ должен быть загружен заранее:
  # docker pull masipcat/wireguard-go:latest
  # docker save masipcat/wireguard-go:latest -o wireguard.tar
EOF
echo "✅ docker-compose.override.yml создан"
echo "   ⚠️  Примечание: WireGuard контейнер использует образ masipcat/wireguard-go:latest"
echo "   ℹ️  Для offline развертывания загрузите образ заранее или используйте локальный образ"
echo ""
...
# Проверка WireGuard конфига
# Конфиг монтируется в WireGuard контейнер через volume
if [ ! -d "wg" ]; then
    mkdir -p wg
    echo "✅ Директория wg/ создана"
    echo "   ⚠️  ВАЖНО: Создайте файл wg/TGBOT.conf с конфигурацией WireGuard"
    echo "   ℹ️  Конфиг будет смонтирован в WireGuard контейнер при запуске"
else
    echo "✅ Директория wg/ уже существует"
    if [ -f "wg/TGBOT.conf" ]; then
        echo "   ✅ Конфиг WireGuard найден: wg/TGBOT.conf (будет смонтирован в WireGuard контейнер)"
    else
        echo "   ⚠️  Конфиг WireGuard не найден: создайте wg/TGBOT.conf"
        echo "   ℹ️  WireGuard контейнер не запустится без конфига"
    fi
fi
echo ""
