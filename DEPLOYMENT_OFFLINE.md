...
**ВАЖНО:** Проект работает ТОЛЬКО в Docker контейнерах. Все необходимые пакеты (`wireguard-tools`, `iproute2`, `openresolv`) установлены в Dockerfile.
...
- Контейнер использует модуль ядра хоста через `/dev/net/tun` (смонтирован в docker-compose.yml)

**Настройка:**
1. Создайте файл `wg/TGBOT.conf` с конфигурацией WireGuard на хосте
2. Контейнер автоматически поднимет WireGuard интерфейс при старте (если конфиг найден)
3. Админ-панель → «Проверить WireGuard» — проверка статуса и управление интерфейсом
