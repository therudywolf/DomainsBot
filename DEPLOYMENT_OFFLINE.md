...
WireGuard используется как резервное подключение, когда все GOST endpoints возвращают 504. В Docker WireGuard **не работает** — для него нужны `wg-quick` и права на создание интерфейсов.

- **В Docker:** Кнопка «Проверить WireGuard» покажет «wg не установлен» — это нормально.
- **На хосте:** Установите `wireguard-tools`, создайте `wg/TGBOT.conf`, запускайте бота на хосте. Бот при старте поднимет WG и будет держать его активным.

Админ-панель → «Проверить WireGuard» — проверка статуса и попытка поднять интерфейс.
