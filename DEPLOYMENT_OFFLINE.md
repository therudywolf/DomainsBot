...
**ВАЖНО:** Проект работает ТОЛЬКО в Docker контейнерах. WireGuard теперь работает в **отдельном контейнере** (`masipcat/wireguard-go`), а не внутри контейнера `tgscanner`.
...
- Это устраняет проблемы с правами доступа и зависимостями
...
**Настройка:**
1. Создайте файл `wg/TGBOT.conf` с конфигурацией WireGuard на хосте
