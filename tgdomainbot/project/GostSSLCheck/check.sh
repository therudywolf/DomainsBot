...
check_domain() {
  ...
  # Берём peer-cert + цепочку (таймаут из GOST_CHECK_TIMEOUT, по умолчанию 30с)
  local gost_timeout="${GOST_CHECK_TIMEOUT:-30}"
  local chain
  chain=$(timeout "${gost_timeout}s" openssl s_client -connect "${host}:443" -servername "$host" \
