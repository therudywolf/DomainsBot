    ...
    if ACCESS_DB_FILE.exists():
        try:
            with open(ACCESS_DB_FILE, "r", encoding="utf-8") as f:
                ...
                # Инициализация списка админов, если отсутствует
                if "_admins" not in data:
                    data["_admins"] = []
                return data
...
def has_permission(user_id: int, permission: str) -> bool:
    ...
    """
    # Главный админ и добавленные админы имеют все разрешения
    if is_admin_panel_allowed(user_id):
        return True
...
def get_access_list() -> dict:
    """Получает список всех доступов с разрешениями (без служебных ключей)."""
    db = load_access_db()
    return {k: v for k, v in db.items() if k != "_admins" and isinstance(v, dict)}


def get_admin_list() -> List[int]:
    """Возвращает список ID пользователей-админов (кроме главного)."""
    db = load_access_db()
    admins = db.get("_admins", [])
    return [int(a) for a in admins if str(a).isdigit()]


def add_admin(user_id: int) -> bool:
    """Добавляет пользователя в список админов. Только главный админ."""
    db = load_access_db()
    admins = db.get("_admins", [])
    if user_id not in admins:
        ...
        db["_admins"] = admins
        save_access_db(db)
        add_access(user_id, "")  # Обеспечиваем базовый доступ
        logger.info(f"Админ добавлен: {user_id}")
        return True
    return False
...
def remove_admin(user_id: int) -> bool:
    ...
    if user_id in admins:
        ...
        db["_admins"] = admins
        save_access_db(db)
        logger.info(f"Админ удалён: {user_id}")
        return True
    return False


def is_admin_panel_allowed(user_id: int) -> bool:
    """Проверяет, может ли пользователь открыть админ-панель."""
...
def build_admin_keyboard(user_id: Optional[int] = None) -> types.InlineKeyboardMarkup:
    ...
    # Кнопки добавления/удаления админа — только для главного админа
    if user_id == ADMIN_ID:
        buttons.append([
            types.InlineKeyboardButton(
...
async def admin_add_access(callback: types.CallbackQuery, state: FSMContext):
    if not is_admin_panel_allowed(callback.from_user.id):
        await callback.answer("❌ Только администратор", show_alert=True)
        return
...
async def process_add_access(message: types.Message, state: FSMContext):
    ...
    if not is_admin_panel_allowed(message.from_user.id):
        return
...
async def admin_add_admin_callback(callback: types.CallbackQuery, state: FSMContext):
    """Добавление админа — только для главного админа."""
    if callback.from_user.id != ADMIN_ID:
        await callback.answer("❌ Только главный администратор может добавлять админов", show_alert=True)
        return
    
    await state.set_state(AdminStates.add_admin_waiting)
    await callback.message.answer(
        "👤 Введите TG ID пользователя для назначения админом.\n\n"
        "Пользователь получит полный доступ к админ-панели."
    )
    await callback.answer()


@router.message(AdminStates.add_admin_waiting)
async def process_add_admin(message: types.Message, state: FSMContext):
    ...
    if add_admin(user_id):
        ...
        await message.answer(f"⚠️ Пользователь {user_id} уже является админом.")
...
async def admin_remove_admin_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    await state.set_state(AdminStates.remove_admin_waiting)
    admins = get_admin_list()
    if not admins:
        await callback.message.answer("📋 Список админов пуст.")
        await state.clear()
    else:
        admins_text = ", ".join(str(a) for a in admins)
        await callback.message.answer(
            f"👤 Введите TG ID админа для удаления.\n\n"
            f"Текущие админы: {admins_text}"
        )
    await callback.answer()


@router.message(AdminStates.remove_admin_waiting)
async def process_remove_admin(message: types.Message, state: FSMContext):
    """Обрабатывает удаление админа."""
    if message.from_user.id != ADMIN_ID:
        return
    ...
    if remove_admin(user_id):
        ...
        await message.answer(f"⚠️ Пользователь {user_id} не найден в списке админов.")
...
async def admin_list_access(callback: types.CallbackQuery):
    ...
    for user_id, data in sorted(db.items(), key=lambda x: (int(x[0]) if str(x[0]).isdigit() else 0)):
        ...
        if uid in admin_ids or uid == ADMIN_ID:
            user_info += " *(админ)*"
        if username:
...
async def admin_manage_permissions(callback: types.CallbackQuery, state: FSMContext):
    ...
    # Две опции: выбор пользователя или массовое редактирование
    keyboard = types.InlineKeyboardMarkup(inline_keyboard=[
        [
            types.InlineKeyboardButton(
                text="👤 Выбрать пользователя",
                callback_data="admin_manage_perm_select_user",
            ),
            types.InlineKeyboardButton(
                text="📦 Массовое редактирование",
                callback_data="admin_bulk_permissions",
            ),
        ],
...
@router.callback_query(F.data == "admin_manage_perm_select_user")
async def admin_manage_perm_select_user(callback: types.CallbackQuery, state: FSMContext):
    """Запрашивает ID пользователя для управления разрешениями."""
    if not is_admin_panel_allowed(callback.from_user.id):
        await callback.answer("❌ Только администратор", show_alert=True)
        return
    ...
    db = get_access_list()
    users_list = "👥 *Введите TG ID пользователя:*\n\n"
    for user_id, data in sorted(db.items(), key=lambda x: (int(x[0]) if str(x[0]).isdigit() else 0)):
        username = data.get("username", "")
...
@router.callback_query(F.data == "admin_bulk_permissions")
async def admin_bulk_permissions(callback: types.CallbackQuery):
    """Показывает выбор права для массового редактирования."""
    if not is_admin_panel_allowed(callback.from_user.id):
        await callback.answer("❌ Только администратор", show_alert=True)
        return
    ...
    for perm_key, perm_name in PERMISSIONS.items():
        keyboard_buttons.append([
            types.InlineKeyboardButton(
                text=perm_name,
                callback_data=f"bulk_perm_select_{perm_key}",
            )
        ])
    keyboard_buttons.append([
        types.InlineKeyboardButton(text="🔙 Назад", callback_data="admin_manage_permissions"),
    ])
    
    await callback.message.edit_text(
        "📦 *Массовое редактирование*\n\nВыберите право для изменения у всех пользователей:",
        parse_mode=ParseMode.MARKDOWN,
...
@router.callback_query(F.data.startswith("bulk_perm_select_"))
async def admin_bulk_perm_action(callback: types.CallbackQuery):
    """Показывает действия: у всех убрать / всем добавить."""
    if not is_admin_panel_allowed(callback.from_user.id):
        await callback.answer("❌ Только администратор", show_alert=True)
        return
    
    perm_key = callback.data.replace("bulk_perm_select_", "")
    if perm_key not in PERMISSIONS:
        await callback.answer("❌ Неизвестное право", show_alert=True)
        return
    
    perm_name = PERMISSIONS[perm_key]
    keyboard = types.InlineKeyboardMarkup(inline_keyboard=[
        [
            types.InlineKeyboardButton(
                text="➖ У всех убрать",
                callback_data=f"bulk_perm_remove_{perm_key}",
            ),
            types.InlineKeyboardButton(
                text="➕ Всем добавить",
                callback_data=f"bulk_perm_add_{perm_key}",
            ),
        ],
        [types.InlineKeyboardButton(text="🔙 Назад", callback_data="admin_bulk_permissions")],
    ])
    await callback.message.edit_text(
        f"📦 *Массовое редактирование*\n\nПраво: {perm_name}\n\nВыберите действие:",
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=keyboard,
    )
    await callback.answer()
...
@router.callback_query(F.data.startswith("bulk_perm_remove_"))
async def admin_bulk_perm_remove(callback: types.CallbackQuery):
    """Убирает право у всех пользователей."""
    if not is_admin_panel_allowed(callback.from_user.id):
        await callback.answer("❌ Только администратор", show_alert=True)
        return
    
    perm_key = callback.data.replace("bulk_perm_remove_", "")
    if perm_key not in PERMISSIONS:
        await callback.answer("❌ Неизвестное право", show_alert=True)
        return
    ...
    for user_id in db:
        if str(user_id).isdigit():
            uid = int(user_id)
            if set_user_permission(uid, perm_key, False):
                count += 1
    
    perm_name = PERMISSIONS[perm_key]
    await callback.answer(f"✅ У {count} пользователей убрано право: {perm_name}", show_alert=True)
    await callback.message.edit_text(
        f"✅ Готово.\n\nУ {count} пользователей убрано право: {perm_name}",
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
            [types.InlineKeyboardButton(text="🔙 К управлению разрешениями", callback_data="admin_manage_permissions")],
        ]),
    )


@router.callback_query(F.data.startswith("bulk_perm_add_"))
async def admin_bulk_perm_add(callback: types.CallbackQuery):
    """Добавляет право всем пользователям."""
    if not is_admin_panel_allowed(callback.from_user.id):
        await callback.answer("❌ Только администратор", show_alert=True)
        return
    
    perm_key = callback.data.replace("bulk_perm_add_", "")
    if perm_key not in PERMISSIONS:
        await callback.answer("❌ Неизвестное право", show_alert=True)
        return
    ...
    for user_id in db:
        if str(user_id).isdigit():
            uid = int(user_id)
            if set_user_permission(uid, perm_key, True):
                count += 1
    
    perm_name = PERMISSIONS[perm_key]
    await callback.answer(f"✅ {count} пользователям добавлено право: {perm_name}", show_alert=True)
    await callback.message.edit_text(
        f"✅ Готово.\n\n{count} пользователям добавлено право: {perm_name}",
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
            [types.InlineKeyboardButton(text="🔙 К управлению разрешениями", callback_data="admin_manage_permissions")],
        ]),
...
async def admin_back(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        help_text,
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=build_admin_keyboard(callback.from_user.id),
    )
    await callback.answer()
...
async def handle_text(message: types.Message, state: FSMContext):
    ...
    elif text == "👨‍💼 Админ-панель" and is_admin_panel_allowed(user_id):
        help_text = (
            "👨‍💼 *Админ-панель*\n\n"
